...
State::State() {}
State::State(vector<int> s) { state = s; }

TransitionCol::TransitionCol() {}
TransitionCol::TransitionCol(vector<int> s) { state = s; }
...
bool compareVec::operator()(vector<int> i) { return (i == key); }

comparePair::comparePair(pair<vector<int>, vector<int>> i) : key(i) {}
bool comparePair::operator()(pair<vector<int>, vector<int>> i) {
  return (i.first == key.first && i.second == key.second);
}

compareMatrix::compareMatrix(vector<int> i) : key(i) {}
bool compareMatrix::operator()(TransitionCol *i) { return (i->state == key); }

compareState::compareState(vector<int> i) : key(i) {}
bool compareState::operator()(State *i) {
  // showVector(i->state);
  // showVector(key);
  return (i->state == key);
}

void MarkovChain::showMatrix(bool isLocal) {
  cout << "---------------------------------" << endl;
  if (isLocal) {
    cout << "LOCAL" << endl;
  } else {
...
void MarkovChain::buildTransition(vector<int> currentState,
  ...
  if (!lastState.empty()) {
    auto it = find_if(transitionMatrix.begin(), transitionMatrix.end(),
                      compareMatrix(lastState));
    TransitionCol *transitionCol;
    ...
    auto it2 =
        find_if(transitionCol->nextStates.begin(),
                transitionCol->nextStates.end(), compareState(currentState));
    ...
    if (none_of(transitionSpace.begin(), transitionSpace.end(),
                comparePair(transition))) {
...
float MarkovChain::getProbability(vector<int> currentState,
                                  vector<int> lastState) {
  pair<vector<int>, vector<int>> transition(lastState, currentState);
  if (none_of(transitionSpace.begin(), transitionSpace.end(),
              comparePair(transition))) {
