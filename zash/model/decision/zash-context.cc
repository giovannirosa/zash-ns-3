...
TimePercentage::TimePercentage(int t) { time = t; }
...
bool compareTObj::operator()(TimeObject *t) {
...
bool compareTimes::operator()(TimePercentage *t) { return t->time == key; }

ContextComponent::ContextComponent() {}
ContextComponent::ContextComponent(ConfigurationComponent *c,
                                   AuditComponent *a) {
...
void ContextComponent::checkBuilding(time_t currentDate) {
...
int ContextComponent::calculateTrust(Context *context, User *user) {
  return context->accessWay->weight + context->localization->weight +
         context->time->weight + user->age->weight + context->group->weight;
}

void ContextComponent::calculateTime(Request *req, time_t currentDate) {
  int time = 0;
  ...
  if (it != timeProbs.end()) {
    ...
    auto it2 = find_if(timeObj->times.begin(), timeObj->times.end(),
                       compareTimes(time));
