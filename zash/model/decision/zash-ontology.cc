...
bool compareOnt::operator()(Ontology *o) {
...
bool compareCap::operator()(enums::Enum *o) {
  return o->key == key->action->key;
}

OntologyComponent::OntologyComponent() {}
OntologyComponent::OntologyComponent(ConfigurationComponent *c,
                                     AuditComponent *a) {
...
bool OntologyComponent::verifyOntology(Request *req, time_t currenDate) {
  ...
  if (it != configurationComponent->ontologies.end()) {
    ...
    auto it2 = find_if(ontology->capabilities.begin(),
                       ontology->capabilities.end(), compareCap(req));
