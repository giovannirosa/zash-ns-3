...
AuthorizationComponent::AuthorizationComponent(
    ConfigurationComponent *c, OntologyComponent *o, ContextComponent *ctx,
...
bool AuthorizationComponent::authorizeRequest(
  ...
  if (!ontologyComponent->verifyOntology(req, currentDate) ||
      !contextComponent->verifyContext(req, currentDate,
...
void AuthorizationComponent::checkUsers(time_t currentDate) {
  for (User *user : configurationComponent->users) {
    remove_if(user->rejected.begin(), user->rejected.end(),
              [this, currentDate](time_t time) {
                return difftime(currentDate, time) <
                       configurationComponent->blockInterval;
              });
  }
}
