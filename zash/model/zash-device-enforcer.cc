  ...
  static TypeId tid =
      TypeId("ns3::DeviceEnforcer")
          ...
          .AddAttribute(
              ...
              MakePointerAccessor(&DeviceEnforcer::m_onTime),
          ...
          .AddAttribute(
              ...
              "state.",
              StringValue("ns3::ConstantRandomVariable[Constant=1.0]"),
              MakePointerAccessor(&DeviceEnforcer::m_offTime),
              MakePointerChecker<RandomVariableStream>())
          .AddAttribute(
              "MaxBytes",
...
int64_t DeviceEnforcer::AssignStreams(int64_t stream) {
...
void DeviceEnforcer::StopSending() {
  NS_LOG_FUNCTION(this);
  CancelEvents();
...
void DeviceEnforcer::ScheduleStartEvent() { // Schedules the event to start
                                            // sending data (switch to the "On"
                                            // state)
  NS_LOG_FUNCTION(this);
  ...
  m_startStopEvent =
      Simulator::Schedule(offInterval, &DeviceEnforcer::StartSending, this);
}

void DeviceEnforcer::ScheduleStopEvent() { // Schedules the event to stop
  ...
  m_startStopEvent =
      Simulator::Schedule(onInterval, &DeviceEnforcer::StopSending, this);
}
...
void DeviceEnforcer::SendPacket() {
  ...
  if (m_unsentPacket) {
    ...
    NS_LOG_INFO("Creating packet with " + to_string(message.size()) +
                " size and '" + message + "' message");
