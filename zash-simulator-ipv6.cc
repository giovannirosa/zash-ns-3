...
DeviceComponent *buildServerStructure(stringstream &fileSim, string simDate,
  ...
  fileSim << "Users of simulation are " << users.size() << ":" << endl;
  for (User *user : users) {
    fileSim << *user << endl;
  }
  ...
  fileSim << "Devices of simulation are " << devices.size() << ":" << endl;
  ...
  fileSim << "Ontologies of simulation are " << ontologies.size() << ":"
          << endl;
  for (Ontology *ontology : ontologies) {
    fileSim << *ontology << endl;
  }
  ...
  fileSim << "Other configuration parameters of simulation are:" << endl;
  fileSim << "Block Threshold: " << blockThreshold << endl;
  fileSim << "Block Interval: " << blockInterval << endl;
  fileSim << "Build Interval: " << buildInterval << endl;
...
void scheduleMessages(NodeContainer staNodes, vector<Device *> devices,
                      DataComponent *dataComponent, stringstream &fileSim,
  ...
  fileSim << "Devices interaction have the following properties: " << endl;
  fileSim << "User: " << user << endl;
  fileSim << "Access Way: " << accessWay << endl;
  fileSim << "Localization: " << localization << endl;
  fileSim << "Group: " << group << endl;
  fileSim << "Action: " << action << endl;

  int idReq = 0;

  NS_LOG_INFO("Opening dataset...");
  string dataset = "data/d6_2m_0tm.csv";
  fileMsgs << "Dataset is: " << dataset << endl;
  ...
  while (csv.FetchNextRow()) {
    ...
    fileMsgs << vecToStr(currentState) << endl;
    ...
    if (lastState.size() > 0) {
      ...
      for (uint32_t change : changes) {
        ...
        fileMsgs << formatTime(currentDate) << " - " << actStr << endl;
        ...
        fileMsgs << "Device enforcer scheduled with message = " << request
                 << endl;
...
int main(int argc, char *argv[]) {
  ...
  Config::SetDefault("ns3::LogDistancePropagationLossModel::ReferenceLoss",
                     DoubleValue(40.046));
  fileSim << "Wifi standard is " << wifiStandard << endl;
  ...
  fileSim << "Number of servers is " << nServers << endl;
  fileSim << "Number of APs is " << nAps << endl;
  fileSim << "Number of STAs is " << N << endl;
  ...
  fileSim << "Server IP is " << serverApInterface.GetAddress(0, 0) << endl;
  ...
  fileSim << "AP IP is " << apInterface.GetAddress(0, 0) << endl;
  ...
  fileSim << "STA IP start with " << staInterface.GetAddress(0, 0) << endl;
  ...
  scheduleMessages(staNodes, devices, dataComponent, fileSim, fileMsgs);
