...
int main(int argc, char *argv[]) {
  ...
  // UintegerValue(250)); Config::SetDefault("ns3::OnOffApplication::DataRate",
  // StringValue("5kb/s"));
  double start = 0.0;
  double stop = 1000.0;
  ...
  // wifiPhy.Set("MaxSupportedRxSpatialStreams", UintegerValue(3));
  ...
  NodeContainer serverAp0 =
      NodeContainer(serverNode.Get(0), apNode.Get(0));
  NodeContainer serverAp1 =
      NodeContainer(serverNode.Get(0), apNode.Get(1));
  ...
  bridgeDev.Add(
      bridge.Install(apNode.Get(1), NetDeviceContainer(apDevice.Get(1),
  ...
  Ipv4InterfaceContainer serverApInterface =
      address.Assign(serverAp0Device.Get(0));
  ...
  while (csv.FetchNextRow()) {
    ...
    if (lastState.size() > 0) {
      ...
      for (int change : changes) {
        ...
        // CreateObject<DeviceEnforcer>(); DeviceEnforcerApp->SetAttribute(
        //     "Protocol", TypeIdValue(TcpSocketFactory::GetTypeId()));
        // DeviceEnforcerApp->SetAttribute("Local", AddressValue(nodeAddress));
        ...
        // DeviceEnforcerApp->SetAttribute("DataRate",
        //                                 DataRateValue(DataRate(dataRate)));
        // DeviceEnforcerApp->SetDeviceName(devices[change]->name);
        // DeviceEnforcerApp->SetMessage(request);
  ...
  if (pcapTracing) {
    ...
    csma.EnablePcap("Server0", serverAp0Device);
    csma.EnablePcap("Server1", serverAp1Device);
  }
